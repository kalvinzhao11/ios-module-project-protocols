...
enum Cards: Int{
    ...
    static var allRanks: [Cards] = [.ace, .two, .three, .four, .five, .six, .seven, .eight, .nine, .ten, .jack, .queen, .king]
}
...
struct Deck{
    ...
    init() {
        for ranking in Cards.allRanks{
            for suitInCard in suit.allSuits{
                cards.append(Card(rank: ranking, suits: suitInCard))
            }
        }
    }
    func drawCard() -> Card {
        let position = Int.random(in: 0...cards.count - 1)
